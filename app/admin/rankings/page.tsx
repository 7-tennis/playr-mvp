...
type RankingRow = {
  ranking_profile_id: string; public_display_name: string; ranking_category: string; development_stage: string | null;
  player_classification: string; publication_status: string; public_region: string | null; safeguarding_hidden: boolean;
  organisation_summary: string | null; rating_value: number | null; participation_score: number; events_played: number;
  matches_played: number; member_status: string; created_at: string; updated_at: string; total_count: number;
};
...
export default async function RankingAdministrationPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const category = ["red", "orange", "green", "open"].includes(searchParams?.category ?? "") ? searchParams?.category ?? null : null;
  const classification = ["junior", "adult"].includes(searchParams?.classification ?? "") ? searchParams?.classification ?? null : null;
  const search = searchParams?.search?.trim().slice(0, 100) || null;
  const rankingResult = await supabase.rpc("get_admin_player_ranking_profiles", {
    p_category: category,
  ...
  const selected = rankings.find((row) => row.ranking_profile_id === searchParams?.record) ?? null;
  const auditResult = selected ? await supabase.rpc("get_admin_player_ranking_audit", { p_limit: 30, p_ranking_profile_id: selected.ranking_profile_id }) : { data: [] };
  ...
  return (
    <PageShell eyebrow="SupeR UseR" subtitle="Approve junior publication and remove unsafe records without exposing private player data." title="Ranking Administration">
      ...
      {rankingResult.error ? <section className="empty-state"><h2 className="section-title">Ranking administration is temporarily unavailable.</h2><p className="mt-2 text-sm text-slate-600">Please try again shortly.</p></section> : rankings.length ? <div className="space-y-3"><p className="text-xs font-bold text-slate-500">Showing {rankings.length} of {rankings[0].total_count} matching records (maximum 100).</p>{rankings.map((row) => <article className="surface-card grid gap-3 p-4 lg:grid-cols-[minmax(0,1fr)_auto]" key={row.ranking_profile_id}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate font-black text-court-navy">{row.public_display_name}</h2><span className={row.safeguarding_hidden ? "ui-chip ui-chip-error" : row.publication_status === "approved" ? "ui-chip ui-chip-success" : "ui-chip ui-chip-warning"}>{row.safeguarding_hidden ? "Hidden" : formatLabel(row.publication_status)}</span></div><p className="mt-2 text-sm font-semibold text-slate-600">{formatLabel(row.player_classification)} · {formatLabel(row.development_stage ?? "open")} stage · {formatLabel(row.ranking_category)} ranking</p><p className="mt-1 text-sm text-slate-600">{row.organisation_summary ?? "No verified active organisation"} · {row.public_region ?? "Region unavailable"}</p><p className="mt-1 text-xs font-semibold text-slate-500">{row.ranking_category === "red" || row.ranking_category === "orange" ? `${row.participation_score ?? 0} participation points` : `${row.rating_value ?? "No"} rating`} · {row.events_played ?? 0} events · Updated {formatDateTime(row.updated_at)}</p></div><Link className="btn-secondary min-h-11 justify-center" href={`/admin/rankings?status=${status}&record=${row.ranking_profile_id}`}>Review and audit</Link></article>)}</div> : <section className="empty-state"><StatusIcon size={24} /><h2 className="section-title mt-3">No ranking records match these filters.</h2></section>}
      {selected ? <section aria-labelledby="ranking-review" className="mt-8 surface-card p-5"><h2 className="section-title" id="ranking-review">Review {selected.public_display_name}</h2><p className="mt-2 text-sm text-slate-600">Actions are authorised in the database and recorded in immutable audit history. Reasons remain private.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{actionForm(selected.ranking_profile_id, "approve", "Approve publication")}{actionForm(selected.ranking_profile_id, "reject", "Return for correction", true)}{actionForm(selected.ranking_profile_id, "hide", "Hide immediately", true)}{actionForm(selected.ranking_profile_id, "suspend", "Suspend publication", true)}{actionForm(selected.ranking_profile_id, "restore", "Restore publication")}</div><h3 className="mt-8 font-black text-court-navy">Audit history</h3><ol className="mt-3 space-y-2">{audit.length ? audit.map((entry, index) => <li className="rounded-playr-md border border-playr-border-subtle p-3 text-sm" key={`${entry.changed_at}-${index}`}><p className="font-black text-court-navy">{formatLabel(entry.action)} · {formatLabel(entry.previous_status ?? "none")} → {formatLabel(entry.new_status ?? "none")}</p><p className="mt-1 text-slate-600">{entry.actor_display_name} · {formatDateTime(entry.changed_at)}</p>{entry.internal_reason ? <p className="mt-1 font-semibold text-slate-700">Private reason: {entry.internal_reason}</p> : null}</li>) : <li className="ui-empty-card">No audit history is available for this record.</li>}</ol></section> : null}
    </PageShell>
  );
}
