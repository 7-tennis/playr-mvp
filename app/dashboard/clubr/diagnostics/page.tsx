...
export default async function ClubRDiagnosticsPage({ searchParams }: { searchParams?: { court?: string; booking?: string } }) {
  ...
  return (
    <ClubRPageFrame context={context} subtitle="Why does ClubR consider this court or booking available?" title="Diagnostics" venue={venue}>
      ...
      <section className="surface-card mb-4 p-4 sm:p-5">
        <form className="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end"><label className="text-sm font-bold text-slate-700">Court<select className="mt-1.5 w-full rounded border border-slate-300 px-3 py-2.5 focus-ring" defaultValue={selectedCourt?.id} name="court">{courts.data.map((court) => <option key={court.id} value={court.id}>{court.name}</option>)}</select></label><label className="text-sm font-bold text-slate-700">Booking ID<input className="mt-1.5 w-full rounded border border-slate-300 px-3 py-2.5 focus-ring" defaultValue={searchParams?.booking ?? ""} name="booking" placeholder="Optional UUID" /></label><button className="btn-primary" type="submit">Inspect</button></form>
      </section>
