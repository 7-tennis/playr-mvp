...
export default async function ClubRDiagnosticsPage({ searchParams }: { searchParams?: { court?: string; booking?: string; profile?: string } }) {
  const { content, context, venue } = await getProtectedClubRPage("clubr:diagnostics");
  if (content) return content;
  if (!context) return null;
  const { dayStart, dayEnd } = dayRange();
  const [courts, occupancy, blocks, settings, booking] = await Promise.all([
  ...
  const error = courts.error ?? occupancy.error ?? blocks.error ?? settings.error ?? booking.error;
  ...
  const relationshipResult = venue && searchParams?.profile
    ? await context.supabase.rpc("playr_resolve_venue_relationship", { p_profile_id: searchParams.profile, p_user_id: context.user.id, p_venue_id: venue.id })
  ...
  return (
    <ClubRPageFrame context={context} subtitle="Why does ClubR consider this court or booking available?" title="Diagnostics" venue={venue}>
      ...
      <section className="surface-card mb-4 p-4 sm:p-5">
        <form className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-end"><label className="text-sm font-bold text-slate-700">Court<select className="mt-1.5 w-full rounded border border-slate-300 px-3 py-2.5 focus-ring" defaultValue={selectedCourt?.id} name="court">{courts.data.map((court) => <option key={court.id} value={court.id}>{court.name}</option>)}</select></label><label className="text-sm font-bold text-slate-700">Booking ID<input className="mt-1.5 w-full rounded border border-slate-300 px-3 py-2.5 focus-ring" defaultValue={searchParams?.booking ?? ""} name="booking" placeholder="Optional UUID" /></label><label className="text-sm font-bold text-slate-700">Profile ID<input className="mt-1.5 w-full rounded border border-slate-300 px-3 py-2.5 focus-ring" defaultValue={searchParams?.profile ?? ""} name="profile" placeholder="Optional UUID" /></label><button className="btn-primary" type="submit">Inspect</button></form>
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-2">
        ...
        <article className="surface-card p-4"><h2 className="section-title">Configuration</h2><dl className="mt-3 grid gap-2 text-sm"><div className="rounded bg-slate-50 p-3"><dt className="font-bold text-slate-500">Venue visibility</dt><dd className="font-black text-court-navy">{venue?.discovery_visibility ?? "Unknown"}</dd></div><div className="rounded bg-slate-50 p-3"><dt className="font-bold text-slate-500">Booking hours</dt><dd className="font-black text-court-navy">{settings.data ? `${settings.data.opening_time.slice(0, 5)}-${settings.data.closing_time.slice(0, 5)}` : "Not configured"}</dd></div><div className="rounded bg-slate-50 p-3"><dt className="font-bold text-slate-500">Guest booking</dt><dd className="font-black text-court-navy">{settings.data?.non_member_booking_enabled ? `Enabled · ${settings.data.guest_advance_booking_days} days ahead` : "Disabled"}</dd></div><div className="rounded bg-slate-50 p-3"><dt className="font-bold text-slate-500">Availability visibility</dt><dd className="font-black text-court-navy">{settings.data?.public_availability_visibility ?? "Not configured"}</dd></div></dl></article>
      </section>
      ...
      {searchParams?.profile ? <section className="surface-card mt-4 p-4"><h2 className="section-title">Venue relationship</h2><pre className="mt-3 overflow-x-auto rounded bg-slate-950 p-4 text-xs text-slate-100">{JSON.stringify(relationshipResult.error ? { error_code: relationshipResult.error.code, resolved: false } : { resolved: true, relationship: relationshipResult.data }, null, 2)}</pre></section> : null}
    </ClubRPageFrame>
  );
}
