...
export default async function TeamREventDetailPage({ params, searchParams }: { params: { eventId: string }; searchParams?: { error?: string; message?: string; q?: string } }) {
  ...
  const participationMutable = event.status === "published" && !event.archived_at && new Date(startsAt).getTime() >= Date.now();
  ...
  const invitationCount = operations.players.filter((player) => player.participation_status === "invited").length;
  const requestCount = operations.players.filter((player) => player.participation_status === "entry_requested").length;
  const declinedCount = operations.players.filter((player) => player.participation_status === "declined").length;
  ...
  return <TeamRPageFrame context={context} subtitle={`${event.host?.name ?? "Organisation"} event`} title={event.title} venue={venue}>
    ...
    <section className="surface-card p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="section-kicker">{event.host?.name}</p><h2 className="section-title mt-1">Event details</h2></div><div className="flex gap-2"><span className="ui-chip ui-chip-brand capitalize">{event.visibility}</span><span className="ui-chip capitalize">{state}</span></div></div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="font-black text-court-navy">Date and time</dt><dd className="mt-1 text-slate-600">{formatDate(startsAt)} · {formatTime(startsAt)}–{formatTime(endsAt)} SAST</dd></div><div><dt className="font-black text-court-navy">Location</dt><dd className="mt-1 text-slate-600">{event.location ?? "To be confirmed"}</dd></div><div><dt className="font-black text-court-navy">Stage/category</dt><dd className="mt-1 text-slate-600">{event.junior_stage ? formatLabel(event.junior_stage) : "Mixed / General"}</dd></div><div><dt className="font-black text-court-navy">Capacity</dt><dd className="mt-1 text-slate-600">{operations.confirmedCount}{event.capacity ? ` / ${event.capacity}` : ""} confirmed</dd></div><div className="sm:col-span-2"><dt className="font-black text-court-navy capitalize">{event.visibility} event</dt><dd className="mt-1 text-slate-600">{eventVisibilityDescription(event.visibility)}</dd></div>{event.description ? <div className="sm:col-span-2"><dt className="font-black text-court-navy">Description</dt><dd className="mt-1 whitespace-pre-wrap leading-6 text-slate-600">{event.description}</dd></div> : null}</dl></section>

    <section className="surface-card mt-4 p-4 sm:p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="section-kicker">Participation</p><h2 className="section-title mt-1">Participants</h2></div><div className="flex flex-wrap gap-2"><span className="ui-chip ui-chip-success">Confirmed {operations.confirmedCount}{event.capacity ? `/${event.capacity}` : ""}</span>{invitationCount > 0 ? <span className="ui-chip ui-chip-warning">Invited {invitationCount}</span> : null}{requestCount > 0 ? <span className="ui-chip ui-chip-brand">Requests {requestCount}</span> : null}{declinedCount > 0 ? <span className="ui-chip ui-chip-muted">Declined {declinedCount}</span> : null}</div></div>
      <div className="mt-4 grid gap-2">{operations.players.map((player) => <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-slate-200 p-3" key={player.assignment_id}><div><p className="font-black text-court-navy">{player.player_name}</p><p className="text-xs font-semibold text-slate-500">{player.junior_stage ? formatLabel(player.junior_stage) : player.is_junior ? "Stage not confirmed" : "Adult"} · {player.context_name} · {player.participation_status === "entry_requested" ? "Entry requested" : formatLabel(player.participation_status)}</p></div>{operations.canManagePlayers && participationMutable ? <div className="flex flex-wrap gap-2">{player.participation_status === "entry_requested" ? <><form action={reviewEventEntryRequest}><input name="eventId" type="hidden" value={event.id} /><input name="assignmentId" type="hidden" value={player.assignment_id} /><input name="decision" type="hidden" value="approve" /><SubmitButton pendingText="Approving…">Approve</SubmitButton></form><form action={reviewEventEntryRequest}><input name="eventId" type="hidden" value={event.id} /><input name="assignmentId" type="hidden" value={player.assignment_id} /><input name="decision" type="hidden" value="reject" /><SubmitButton className="btn-secondary" pendingText="Rejecting…">Reject</SubmitButton></form></> : null}{["invited", "confirmed"].includes(player.participation_status) ? <form action={removeEventPlayerAssignment}><input name="eventId" type="hidden" value={event.id} /><input name="assignmentId" type="hidden" value={player.assignment_id} /><SubmitButton className="btn-secondary" pendingText="Removing…">Remove</SubmitButton></form> : null}</div> : null}</div>)}{operations.players.length === 0 ? <div className="ui-empty-card">No participation activity yet. Eligibility alone does not occupy capacity.</div> : null}</div>
      {operations.canManagePlayers && participationMutable && event.visibility === "closed" ? <div className="mt-5 border-t border-slate-200 pt-4"><h3 className="font-black text-court-navy">Invite Player</h3><form action={assignEventPlayer} className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end"><input name="eventId" type="hidden" value={event.id} /><label className="min-w-0 flex-1 text-sm font-bold text-court-navy">Eligible player<select className={fieldClass} name="playerProfileId" required><option value="">Choose player</option>{operations.playerCandidates.map((player) => <option key={player.player_profile_id} value={player.player_profile_id}>{player.player_name} · {player.junior_stage ? formatLabel(player.junior_stage) : player.is_junior ? "Stage not confirmed" : "Adult"}</option>)}</select></label><SubmitButton pendingText="Inviting…">Invite Player</SubmitButton></form>{operations.playerCandidates.length === 0 ? <p className="mt-2 text-xs font-semibold text-slate-500">No additional eligible players match this event.</p> : null}</div> : null}
    </section>
