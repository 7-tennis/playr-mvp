...
import { safeSchoolConnectionsReturnTo, schoolConnectionsHref } from "@/lib/school-connections-navigation";
...
function messageText(message?: string, schoolName?: string) {
  if (message === "pending") return `School request sent. ${schoolName ?? "The school"} will review the request before the School is linked to this player.`;
  if (message === "already_connected") return "School linked. This player is already connected to that School.";
  return null;
}
...
export default async function JuniorSchoolsPage({
  ...
  searchParams?: { error?: string; message?: string; onboarding?: string; q?: string; returnTo?: string; school?: string };
}) {
  const onboarding = searchParams?.onboarding === "1";
  const returnTo = safeSchoolConnectionsReturnTo(searchParams?.returnTo, params.juniorId, onboarding);
  const connectionsPath = schoolConnectionsHref(params.juniorId, { onboarding, returnTo });
  const supabase = await createServerSupabaseClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/login?next=${encodeURIComponent(connectionsPath)}`);
  ...
  const messageSchool = schools.find((school) => school.id === searchParams?.school)?.name;
  const backLabel = onboarding ? "Not now — finish Junior setup" : returnTo === `/dashboard/players/${junior.id}` ? "Back to MyPlayR" : "Back to Juniors";
  ...
  return (
    <PageShell eyebrow="Junior school connections" subtitle={`Connect ${junior.first_name}’s existing PlayR profile to an eligible school.`} title="Find a School">
      <StatusAlert className="mb-4" message={messageText(searchParams?.message, messageSchool)} tone="success" />
      ...
      <form className="surface-card mb-5 flex flex-col gap-3 p-4 sm:flex-row" method="get">
        {onboarding ? <input name="onboarding" type="hidden" value="1" /> : null}
        {!onboarding && returnTo !== "/dashboard/juniors" ? <input name="returnTo" type="hidden" value={returnTo} /> : null}
        ...
        <div className="flex items-end gap-2"><button className="btn-primary px-4 py-2" type="submit">Search</button>{queryText ? <Link className="btn-secondary px-4 py-2" href={connectionsPath}>Clear</Link> : null}</div>
      </form>
              ...
              <div className="mt-auto pt-4">
                {link ? <span className={`ui-chip ${link.status === "active" ? "ui-chip-success" : "ui-chip-muted"}`}>{link.status === "active" ? "Connected" : link.status === "pending" ? "Approval pending" : formatLabel(link.status)}</span> : (
                  <form action={requestJuniorSchoolConnection}>
                    ...
                    <input name="onboarding" type="hidden" value={onboarding ? "1" : "0"} />
      ...
      <Link className="btn-secondary mt-5" href={returnTo}>{backLabel}</Link>
    </PageShell>
  );
}
